 ...
 */
package de.hybris.beerstore.cockpits.setup;
...
import de.hybris.beerstore.cockpits.constants.BeerstoreCockpitsConstants;
 ...
 */
@SystemSetup(extension = BeerstoreCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(BeerstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(BeerstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						BeerstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/beerstorecockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
